...
def main(command_line_parameters=None):
  ...
  try:
    ...
    pdf.savefig(_plot_cmc(cmcs_dev, colors, args.legends, "NIVL CMC between 5 splits", args.legend_font_size, args.legend_position))
  ...
  if args.roc or args.det:
    ...
    if args.roc:
      ...
      try:
        # create a separate figure for dev and eval
        pdf.savefig(_plot_roc(scores_dev, colors, args.legends, "NIVL ROC Curve between 5 splits", args.legend_font_size, args.legend_position))
        #del frrs_dev
      except RuntimeError as e:
        raise RuntimeError("During plotting of ROC curves, the following exception occured:\n%s\nUsually this happens when the label contains characters that LaTeX cannot parse." % e)
    ...
    if args.det:
      ...
      try:
        # create a separate figure for dev and eval
        pdf.savefig(_plot_det(scores_dev, colors, args.legends, "NIVL DET between 5 splits", args.legend_font_size, args.legend_position))
        #del dets_dev
      except RuntimeError as e:
        raise RuntimeError("During plotting of ROC curves, the following exception occured:\n%s\nUsually this happens when the label contains characters that LaTeX cannot parse." % e)
